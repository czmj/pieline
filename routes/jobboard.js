...
exports.list = function(req, res){
  ...
  req.getConnection(function(err,connection){
        ...
        var query = connection.query('SELECT j.*, c.company_name AS company, l.location_name AS location FROM jobboard j INNER JOIN companies c ON j.companyID=c.companyID INNER JOIN locations l ON j.locationID=l.locationID WHERE dateposted between DATE("' + month_ago_today + '") AND DATE("' + today + '") ORDER BY dateposted DESC, id DESC',function(err,rows)
        {
               if(!rows[0]){
                        res.status(404);
...
exports.city = function(req, res){
  ...
  req.getConnection(function(err,connection){
	...
	var query = connection.query('SELECT j.*, c.company_name AS company, l.location_name AS location FROM jobboard j INNER JOIN companies c ON j.companyID=c.companyID INNER JOIN locations l ON j.locationID=l.locationID WHERE l.location_name = ? AND dateposted between DATE("' + month_ago_today + '") AND DATE("' + today + '") ORDER BY dateposted DESC, id DESC',[city],function(err,rows)
        {
                if(!rows[0]){
...
exports.citycategory = function(req, res){
  ...
  req.getConnection(function(err,connection){
        ...
        var query = connection.query('SELECT j.*, c.company_name AS company, l.location_name AS location, cat.category_name AS category FROM jobboard j INNER JOIN companies c ON j.companyID=c.companyID INNER JOIN locations l ON j.locationID=l.locationID INNER JOIN categories cat ON j.categoryID=cat.categoryID WHERE l.location_name = ? AND cat.category_name = ? AND dateposted between DATE("' + month_ago_today + '") AND DATE("' + today + '") ORDER BY dateposted DESC, id DESC',[city,category],function(err,rows)
        {
                if(!rows[0]){
